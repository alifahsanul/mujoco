...
class ArmEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        radius = 0.36
        minimize_radius = 0.2 # lower to reduce randomness of x position
    ...
    def _get_obs(self):
        ...
        # dof_vel = dof_vel / np.array([2])  # Assuming the range for dof_vel is [-1.5, 1.5]
        # qacc = qacc / np.array([6])
        ...
        obs = np.concatenate([prev_action, arm_tip_pos, ball_pos, dof_vel, qacc])
        return obs
    ...
    def _calculate_reward(self, action):
        ...
        # Discourage big change in action
        action_diff_penalty = np.linalg.norm(action - self.previous_action)
        
        reward = - (distance ** 2) * 1 \
                 - (velocity_penalty ** 2) * 1 \
                 - action_diff_penalty * 1 \
                 - time_penalty * 0.01 \
